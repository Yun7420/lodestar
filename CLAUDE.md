...
1. **`docs/ROADMAP.md`** — **최상단 `🎯 NEXT SESSION` 섹션에 지금 시점 우선 후보 4개(A~D)와 추천 순서가 정리되어 있음**. 사용자가 "이어서 하자" 같은 재개 신호 주면 이 후보들을 `AskUserQuestion`으로 제안.
2. **`docs/AI_HANDOFF.md`** — 컨셉·유저 취향·코드 지도·결정 이력. 특히 §6 유저 취향은 안 읽으면 이전 결정을 되돌리게 됩니다.
